...
export function exsimApp() {
  ...
  const handleStart = async () => {
    ...
    formData.append('remove_first_page', removeFirstPage.toString());
  ...
  const startWebSocket = (sessionId) => {
    ...
    const ws = new WebSocket(`${WS_BASE}/ws/exsim-progress/${sessionId}`);
    wsRef.current = ws;
  ...
  return (
    <div className="min-h-screen bg-transparent flex flex-col items-center py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="w-full max-w-5xl mb-8 text-center">
        ...
        <h1 className="text-3xl font-light text-black dark:text-white tracking-tight">exsim <span className="font-semibold">Automation</span></h1>
        <p className="mt-3 text-lg text-gray-500 dark:text-gray-400">Generate and merge your exsim invoices instantly</p>
      </div>
          ...
          <DropZone 
            title={
              <div className="flex justify-between items-center">
                ...
                <a href="/sample_exsim.csv" download className="text-xs text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white underline font-normal">
                  Download Sample CSV
                </a>
              </div>
            }
